#!/usr/bin/perl
#****************************************************************#
# ScriptName: mysqlawr.pl
# Create Date: 2014-08-18 13:52
# Modify Date: 2014-08-18 13:52
#***************************************************************#
...
( my $sript_name = $0 ) =~ s!.*/(.*)!$1! ;
my $lock_handle = _obtain_lock($sript_name);
...
Sample :
   shell> perl dbmon.pl -p 111111 -lh 192.168.1.111 -tp 111111 -th 192.168.1.200 -I 11
==========================================================================================
EOF
	exit;
}
...
sub get_mysqlstat {
    ...
    my ($running_thread_threshold,$times_per_hour)=$dbh_save->selectrow_array("select running_thread_threshold,times_per_hour from dbmon.myawr_host where id=$tid");
	...
	foreach (@result) {
        ...
        if($running_thread_threshold<=$now_running_threads  and  $times_saved<$times_per_hour){
		    $sql =qq{insert into dbmon.myawr_engine_innodb_status(snap_id,host_id,order_id,row_status,snap_time)  values($snap_id,$tid,$order_id, \"$_\", \"$snap_time\")};
		    $dbh_save->do($sql);
		    $order_id +=1;
        }
	}
    ...
    $sql =
	qq{update dbmon.myawr_host set uptime="$varuptime",check_time=now() where id=$tid};
    $dbh_save->do($sql);
...
sub get_osstat {
    ...
    $snap_time =`date +"%Y-%m-%d %H:%M:%S"`;
 	...
 	my $sth = $dbh_save->prepare("SELECT max(snap_id) from dbmon.myawr_snapshot where host_id=$tid and snap_id is not null");
	$sth->execute();
    ...
    $sql =qq{insert into dbmon.myawr_snapshot(host_id,snap_time,snap_id) values($tid, \"$snap_time\",$snap_id)};
    $dbh_save->do($sql);

	$sql =qq{insert into dbmon.myawr_load_info(snap_id,host_id,load1,load5,load15,snap_time) values($snap_id,$tid, $sys_load[0],$sys_load[1],$sys_load[2], \"$snap_time\")};
	$dbh_save->do($sql);

	$sql =qq{insert into dbmon.myawr_cpu_info(snap_id,host_id,cpu_user,cpu_system,cpu_idle,cpu_iowait,snap_time) values($snap_id,$tid, $user_diff_1,$system_diff_1,$idle_diff_1,$iowait_diff_1, \"$snap_time\")};
	$dbh_save->do($sql);

	$sql =qq{insert into dbmon.myawr_io_info(snap_id,host_id,rd_ios_s,wr_ios_s,rkbs,wkbs,queue,wait,svc_t,busy,snap_time) values($snap_id,$tid, $rd_ios_s,$wr_ios_s ,$rkbs,$wkbs,$queue,$wait,$svc_t,$busy, \"$snap_time\")};
	$dbh_save->do($sql); 

	$sql =
	qq{insert into dbmon.myawr_swap_net_disk_info(snap_id,host_id,swap_in,swap_out,net_recv,net_send,file_system,total_mb,used_mb,used_pct,mount_point,snap_time) values($snap_id,$tid, $swapin,$swapout,$diff_recv,$diff_send,\"$filesystem\", $totalmb, $usedmb,  $usedpct, \"$mountpoint\" , \"$snap_time\")};
	$dbh_save->do($sql);
...
sub get_perfstat {
    ...
    if ($perfon eq 'ON'){
		...
		while( my @result2 = $sth->fetchrow_array )	{
			    $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
			    $sth2->bind_param( 2, $result2[6], SQL_INTEGER);
			    $sth2->bind_param( 3, $result2[12], SQL_INTEGER );
			    $sth2->bind_param( 4, $result2[11], SQL_INTEGER );
			    $sth2->bind_param( 5, $result2[17], SQL_INTEGER );
...
# ----------------------------------------------------------------------------------------
# 
# Func : get io sys schema performance info
# ----------------------------------------------------------------------------------------    
sub get_iosysperfstat {
	my $dbh = connect_mysql(
		database   => 'information_schema',
		host       => $lhost,
		port       => $port,
    ...
    if ($perfon eq 'ON'){
     ...
     my $sql2 = qq{ insert into myawr_host_summary_by_file_io(snap_id,host_id,host_name,io_sum,io_latency,snap_time) values ($snap_id,$tid,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select host,ios,io_latency from sys.x\$host_summary_by_file_io where host is not null and host NOT IN ('localhost','127.0.0.1')");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
     ...
     my $sql2 = qq{ insert into myawr_user_summary_by_file_io(snap_id,host_id,user_name,io_sum,io_latency,snap_time) values ($snap_id,$tid,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select user,ios,io_latency from sys.x\$user_summary_by_file_io ");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
     ...
     my $sql2 = qq{ insert into myawr_io_read_global_by_file_by_bytes(snap_id,host_id,file,count_read,total_read,avg_read,write_pct,snap_time) values ($snap_id,$tid,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select sys.format_path(file),count_read,total_read,avg_read,write_pct from   sys.x\$io_global_by_file_by_bytes  where file not like '%binlog%' and file not like '%#P#%'  and count_read > 50  order by avg_read  desc  limit 50;");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
     ...
     my $sql2 = qq{ insert into myawr_io_write_global_by_file_by_bytes(snap_id,host_id,file,count_write,total_write,avg_write,write_pct,snap_time) values ($snap_id,$tid,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select sys.format_path(file),count_write,total_written,avg_write,write_pct from   sys.x\$io_global_by_file_by_bytes  where file not  like '%#P#%' and file not like '%binlog%' order by  total_written desc limit 50;");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
     ...
     my $sql2 = qq{ insert into myawr_io_global_file_by_latency(snap_id,host_id,file,total,total_latency,count_read,read_latency,count_write,write_latency,count_misc,misc_latency,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select sys.format_path(file),total,total_latency,count_read,read_latency,count_write,write_latency,count_misc,misc_latency  from sys.x\$io_global_by_file_by_latency limit 10");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
               ...
               $sth2->bind_param( 8, $result2[7], SQL_INTEGER );
               $sth2->bind_param( 9, $result2[8], SQL_INTEGER );
               $sth2->bind_param( 10, $snap_time, SQL_DATE );
               $sth2->execute();
               $dbh_save->commit();
          }


     my $sql2 = qq{ insert into myawr_io_global_event_by_latency(snap_id,host_id,event_name,total,total_latency,avg_latency,max_latency,read_latency,write_latency,misc_latency,total_read,total_written,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select event_name,total,total_latency,avg_latency,max_latency,read_latency,write_latency,misc_latency,total_read,total_written from sys.x\$io_global_by_wait_by_latency  limit 10");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
...
# ----------------------------------------------------------------------------------------
# 
# Func : get latency sys schema performance info
# ----------------------------------------------------------------------------------------    
sub get_latencyperfstat {
	my $dbh = connect_mysql(
		database   => 'information_schema',
		host       => $lhost,
		port       => $port,
    ...
    if ($perfon eq 'ON'){
     ...
     my $sql2 = qq{ insert into myawr_wait_classes_global_by_avg_latency(snap_id,host_id,event_class,total,total_latency,min_latency,avg_latency,max_latency,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select  event_class,total,total_latency,min_latency,avg_latency,max_latency   from sys.wait_classes_global_by_avg_latency");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
     ...
     my $sql2 = qq{ insert into myawr_host_summary_by_statement_latency(snap_id,host_id,host_name,total,total_latency,max_latency,lock_latency,rows_sent,rows_examined,rows_affected,full_scans,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select host,total,total_latency,max_latency,lock_latency,rows_sent,rows_examined,rows_affected,full_scans  from sys.x\$host_summary_by_statement_latency where host is not null and host NOT IN ('localhost','127.0.0.1')");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
     ...
     my $sql2 = qq{ insert into myawr_user_summary_by_statement_latency(snap_id,host_id,user_name,total,total_latency,max_latency,lock_latency,rows_sent,rows_examined,rows_affected,full_scans,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select user,total,total_latency,max_latency,lock_latency,rows_sent,rows_examined,rows_affected,full_scans  from sys.x\$user_summary_by_statement_latency");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_INTEGER );
...
# ----------------------------------------------------------------------------------------
# 
# Func : get statment sys performance info
# ----------------------------------------------------------------------------------------    
sub get_statment {
    my $dbh = connect_mysql(
        database   => 'information_schema',
    ...
    if ($perfon eq 'ON'){
     ...
     my $sql2 = qq{ insert into myawr_statements_with_full_table_scans(snap_id,host_id,query,db,exec_count,total_latency,no_index_used_count,no_good_index_used_count,no_index_used_pct,rows_sent,rows_examined,rows_sent_avg,rows_examined_avg,first_seen,last_seen,digest,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select query,db,exec_count,total_latency,no_index_used_count,no_good_index_used_count,no_index_used_pct,rows_sent,rows_examined,rows_sent_avg,rows_examined_avg,first_seen,last_seen,digest from sys.x\$statements_with_full_table_scans    where query like 'SELECT%' order by  last_seen desc   limit 10");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_LONGVARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_VARCHAR );
               $sth2->bind_param( 3, $result2[2], SQL_INTEGER );
               $sth2->bind_param( 4, $result2[3], SQL_INTEGER );
               $sth2->bind_param( 5, $result2[4], SQL_INTEGER );
               ...
               $sth2->bind_param( 10, $result2[9], SQL_INTEGER );
               $sth2->bind_param( 11, $result2[10], SQL_INTEGER );
               $sth2->bind_param( 12, $result2[11], SQL_DATE );
               $sth2->bind_param( 13, $result2[12], SQL_DATE );
               $sth2->bind_param( 14, $result2[13], SQL_VARCHAR );
               $sth2->bind_param( 15, $snap_time, SQL_DATE );
               $sth2->execute();
               $dbh_save->commit();
          }


     my $sql2 = qq{ insert into myawr_statements_with_temp_tables(snap_id,host_id,query,db,exec_count,total_latency,memory_tmp_tables,disk_tmp_tables,avg_tmp_tables_per_query,tmp_tables_to_disk_pct,first_seen,last_seen,digest,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("select query,db,exec_count,total_latency,memory_tmp_tables,disk_tmp_tables,avg_tmp_tables_per_query,tmp_tables_to_disk_pct,first_seen,last_seen,digest  from sys.x\$statements_with_temp_tables where last_seen > date_sub(now(),interval 1 day) and query like 'SELECT%' and db not in ('NULL','dbmon') order by disk_tmp_tables desc ,memory_tmp_tables  desc  limit 10");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_LONGVARCHAR );
               $sth2->bind_param( 2, $result2[1], SQL_VARCHAR );
               ...
               $sth2->bind_param( 7, $result2[6], SQL_INTEGER );
               $sth2->bind_param( 8, $result2[7], SQL_INTEGER );
               $sth2->bind_param( 9, $result2[8], SQL_DATE );
               $sth2->bind_param( 10, $result2[9], SQL_DATE );
               $sth2->bind_param( 11, $result2[10], SQL_VARCHAR );
               $sth2->bind_param( 12, $snap_time, SQL_DATE );
               $sth2->execute();
               $dbh_save->commit();
          }
...
# ----------------------------------------------------------------------------------------
# 
# Func : get segment sys performance info
# ----------------------------------------------------------------------------------------    
sub get_segment {
    my $dbh = connect_mysql(
        database   => 'information_schema',
    ...
    if ($perfon eq 'ON'){
     ...
     my $sql2 = qq{ insert into myawr_segment_global_stat(snap_id,host_id,object_schema,table_name,index_name,rows_selected,select_latency,rows_inserted,insert_latency,rows_updated,update_latency,rows_deleted,delete_latency,object_type,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );

     $sth = $dbh->prepare("SELECT 
  `psi`.`OBJECT_SCHEMA` AS `table_schema`,
  `psi`.`OBJECT_NAME` AS `table_name`,
  `psi`.`INDEX_NAME` AS `index_name`,
  `psi`.`COUNT_FETCH` AS `rows_selected`,
  `psi`.`SUM_TIMER_FETCH` AS `select_latency`,
  `psi`.`COUNT_INSERT` AS `rows_inserted`,
  `psi`.`SUM_TIMER_INSERT` AS `insert_latency`,
  `psi`.`COUNT_UPDATE` AS `rows_updated`,
  `psi`.`SUM_TIMER_UPDATE` AS `update_latency`,
  `psi`.`COUNT_DELETE` AS `rows_deleted`,
  `psi`.`SUM_TIMER_INSERT` AS `delete_latency` 
FROM
  `performance_schema`.`table_io_waits_summary_by_index_usage`  `psi`
WHERE (
    `psi`.`INDEX_NAME` IS NOT NULL
  ) 
ORDER BY (
    `psi`.`COUNT_FETCH` + `psi`.`COUNT_INSERT` + `psi`.`COUNT_UPDATE` + `psi`.`COUNT_DELETE`
  ) DESC 
LIMIT 10 ");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               ...
               $sth2->bind_param( 10, $result2[9], SQL_INTEGER );
               $sth2->bind_param( 11, $result2[10], SQL_INTEGER );
               $sth2->bind_param( 12, 'I', SQL_VARCHAR );
               $sth2->bind_param( 13, $snap_time, SQL_DATE );
               $sth2->execute();
               $dbh_save->commit();
          }
     ...
     my $sql2 = qq{ insert into myawr_segment_global_stat(snap_id,host_id,object_schema,table_name,rows_selected,select_latency,rows_inserted,insert_latency,rows_updated,update_latency,rows_deleted,delete_latency,object_type,snap_time) values ($snap_id,$tid,?,?,?,?,?,?,?,?,?,?,?,?)};
     my $sth2 = $dbh_save->prepare( $sql2 );
...
ORDER BY (
    `pst`.`COUNT_FETCH` + `pst`.`COUNT_INSERT` + `pst`.`COUNT_UPDATE` + `pst`.`COUNT_DELETE`
  ) DESC   limit 10; ");
     $sth->execute();
     while( my @result2 = $sth->fetchrow_array ) {
               $sth2->bind_param( 1, $result2[0], SQL_VARCHAR );
               ...
               $sth2->bind_param( 9, $result2[8], SQL_INTEGER );
               $sth2->bind_param( 10, $result2[9], SQL_INTEGER );
               $sth2->bind_param( 11, 'T', SQL_VARCHAR );
               $sth2->bind_param( 12, $snap_time, SQL_DATE );
               $sth2->execute();
               $dbh_save->commit();
          }
